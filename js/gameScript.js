...
var mainDeck;
var mainTimer;
var username_g, roomname_g, partner_g, roomcreator_g, isMaker_g;
var ourGamesWin = 0;
var ourGamesLose = 0;
var isGameScreenOn = false;
...
function gameStart(){
    ...
    partner_g = "";
    roomcreator_g = "";
    for(var i = 0 ; i < roomname_g.length-5; i++){
        roomcreator_g = roomcreator_g + roomname_g.charAt(i);
    }
    if(roomcreator_g === username_g){
        isMaker_g = true;
        partner_g = PARTNER;
    }   
	isGameScreenOn = true;
...
function setup(){
    ...
    for(var i = 1; i<=52; i++)
    t = t + i.toString() + " ";
    var namearr = ["me","opponent 2","my partner","opponent 2"];
    this.mainDeck = new Deck(t, 0, namearr);

    this.addSocket();
	console.log(globalThis.username_g +" ---- "+globalThis.roomcreator_g)
	SOCKET.emit("setPlayerReady");
    if(globalThis.username_g === globalThis.roomcreator_g)
	this.waitForUsersToConnect();    
}

function addSocket(){
    
    SOCKET.emit('joinRoom', globalThis.username_g, globalThis.roomname_g);
    console.log("socket start");

    SOCKET.on('incomingUsersConnectedInfo', function(inJson){
        console.log(inJson);
        var jsonObj = JSON.parse(inJson);
        var num = jsonObj.playerCount;
        if(jsonObj.isRoomReady == "true"){
    ...
    SOCKET.on('incomingNewGameInfo', function(info){
        ...
        for(var i = 0; i < 4; i ++){
            if(playerarr[i] === globalThis.username_g)
            pos = i;
        }
...
function makeGameAndSendToAll(inJson){
    ...
    var newGameJson = globalThis.createNewGame(inJson, globalThis.username_g, globalThis.partner_g)
    SOCKET.emit('newGameInfo', newGameJson);
    //MakeGameConfig
    //SendGameConfifToAll
}
...
function getUsersConnectedInfo(){
    SOCKET.emit('getUserConnectedInfo');
}
